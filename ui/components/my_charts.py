...
class ChartWidget(QtWidgets.QWidget):
    def __init__(self, parent, zaur):
        ...
        self.zaur = zaur
        self.axes = dict()
        self.setGeometry(QtCore.QRect(0, 0, 901, 455))
        self.frame = QtWidgets.QFrame(self)
        self.frame.setGeometry(QtCore.QRect(0, 0, 901, 455))
        ...
        self.tabs.setGeometry(QtCore.QRect(0, 0, 901, 455))
        self.tabs.setTabPosition(QtWidgets.QTabWidget.North)
        self.tabs.setMovable(True)

    def ohlc_chart(self, df):
        tab = OHLCChart(self, df)
...
class MultiLineChart(QtWidgets.QWidget):
    ...
    def update_legend(self, x, y):
        ...
        for col in self.df.columns:
            prices.append(col)
            prices.append(str(row[col].__round__(3)))
...
class OHLCChart(QtWidgets.QWidget):
    ...
    def update_legend(self, x, y):
        row = self.df.loc[x / 1000000]
        diff = 100 * (row.Close - row.Open) / row.Open
        fmt = '<span style="color:#%s">%%.2f</span>' % ('097' if (row.Open < row.Close).all() else 'd00')
        text = 'O %s H %s L %s C %s Vol %s Change %s' % (fmt, fmt, fmt, fmt, fmt, fmt)
        self.hover_label.setText(text % (row.Open, row.High, row.Low, row.Close, row.Volume, diff) + '%')
